<template>
  <div class="w-full h-full @container">
    <div class="grid @2xl:flex justify-between u-mb-s">
      <div v-show="item?.username" class="grid">
        ...
        <div class="flex items-center gap-1 my-2 u-text-1">
          ...
          <span class="u-text-1 font-medium ml-2"> {{ `${item?.username}` }}: </span> <span class="ml-1">{{ `
...
const { item, formkitSchema, metadata, formkitFields } = storeToRefs(store)


const { onResult, loading } = apollo.query('getMetadataResource', { entity: 'User' });
...
onResult(({ data, networkStatus }) => {
  ...
  // store.resource({ username: route.params.username })
  // log(formkitFields.value.fields)
  const { onResult, loading } = apollo.query('getUserByUsernameUser', { username: route.params.username }, formkitFields.value)

  onResult(({ data, networkStatus }) => {
    if (data) {
      const { y: scrollY } = useWindowScroll();
      scrollY.value = 0;
      log(data.getUserByUsernameUser)
      item.value = useCloned(data.getUserByUsernameUser).cloned.value;
      item.value.localidad = '/api/localidads/1'
    }
  });
});
...
function remove() {
  chanel = `${item.value?.username}_remove`
  msgbus('remove').emit({ chanel: chanel, header: 'Eliminar usuario', message: `Desea eliminar el usuario <b>${item.value?.username}</b> ?` })
