<template>
  <nuxt-link :to="{ name: 'paradas' }" class="text-blue-600 hover:text-blue-800">
    &lt; Back to list
  </nuxt-link>

  <h1 class="text-3xl my-4">Create Parada</h1>

  <div
    v-if="isLoading"
    class="bg-blue-100 rounded py-4 px-4 text-blue-700 text-sm"
    role="status"
  >
    Loading...
  </div>

  <div
    v-if="error"
    class="bg-red-100 rounded py-4 px-4 my-2 text-red-700 text-sm"
    role="alert"
  >
    {{ error }}
  </div>

  <Form :errors="violations" @submit="create" />
</template>

<script lang="ts" setup>
import { onBeforeUnmount } from "vue";
import { storeToRefs } from "pinia";
import Form from "~~/components/parada/ParadaForm.vue";
import { useParadaCreateStore } from "~~/stores/parada/create";
import { useCreateItem } from "~~/composables/api";
import { getIdFromIri } from "~~/utils/resource";
import type { Parada } from "~~/types/parada";

const paradaCreateStore = useParadaCreateStore();
const { created, isLoading, violations, error } = storeToRefs(paradaCreateStore);

async function create(item: Parada) {
  const data = await useCreateItem<Parada>("api/paradas", item);
  paradaCreateStore.setData(data);

  if (!created?.value?.["@id"]) {
    paradaCreateStore.setError("Missing item id. Please reload");
    return;
  }

  navigateTo({
    name: "paradas-id-edit",
    params: { id: getIdFromIri(created?.value?.["@id"]) },
  });
}

onBeforeUnmount(() => {
  paradaCreateStore.$reset();
});
</script>
