<template>
  ...
  <h1 class="text-3xl my-4">Show MenuItem {{ item?.["@id"] }}</h1>
...
import { useMenuItemShowStore } from "~~/stores/menuitem/show";
...
const { error: deleteError, deleted } = storeToRefs(menuitemDeleteStore);

const menuitemShowStore = useMenuItemShowStore();

useMercureItem({
  store: menuitemShowStore,
...
menuitemShowStore.setData({ retrieved: item, isLoading, error, hubUrl });
