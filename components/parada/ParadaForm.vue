<template>
  <form class="py-4" @submit.prevent="emitSubmit">
    ...
    <div class="mb-2">
      <label
        for="parada_recorridos"
        class="text-gray-700 block text-sm font-bold capitalize"
      >
        recorridos
      </label>
      <FormRepeater
        :values="item.recorridos"
        @update="(values: any[]) => (item.recorridos = values)"
      />
      <div
        v-if="violations?.recorridos"
        class="bg-red-100 rounded py-4 px-4 my-2 text-red-700 text-sm"
      >
