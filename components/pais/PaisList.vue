<template>
  <div class="flex items-center justify-between">
    <h1 class="text-3xl my-4">Pais List</h1>

    <nuxt-link
      :to="{ name: 'paiss-create' }"
      ...
      <tbody>
        <tr v-for="item in items" :key="item['@id']" class="border-b">
          <td class="px-6 py-4 text-sm">
            <nuxt-link
              :to="{ name: 'paiss-id', params: { id: getIdFromIri(item['@id']) } }"
  ...
  <div v-if="view" class="flex justify-center">
    <nav aria-label="Page navigation">
      <ul class="flex list-style-none">
        ...
        <li :class="{ disabled: !pagination.next }">
          <nuxt-link
            :to="{ name: 'paiss-page-page', params: { page: pagination.last } }"
            :class="
              !pagination.next
                ? 'text-gray-500 pointer-events-none'
                : 'text-gray-800 hover:bg-gray-200'
...
import { usePaisListStore } from "~~/stores/pais/list";
...
useMercureList({ store: paisListStore, deleteStore: paisDeleteStore });
