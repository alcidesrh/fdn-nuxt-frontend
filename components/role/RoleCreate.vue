<template>
  <nuxt-link :to="{ name: 'roles' }" class="text-blue-600 hover:text-blue-800">
    &lt; Back to list
  </nuxt-link>

  <h1 class="text-3xl my-4">Create Role</h1>

  <div
    v-if="isLoading"
    class="bg-blue-100 rounded py-4 px-4 text-blue-700 text-sm"
    role="status"
  >
    Loading...
  </div>

  <div
    v-if="error"
    class="bg-red-100 rounded py-4 px-4 my-2 text-red-700 text-sm"
    role="alert"
  >
    {{ error }}
  </div>

  <Form :errors="violations" @submit="create" />
</template>

<script lang="ts" setup>
import { onBeforeUnmount } from "vue";
import { storeToRefs } from "pinia";
import Form from "~~/components/role/RoleForm.vue";
import { useRoleCreateStore } from "~~/stores/role/create";
import { useCreateItem } from "~~/composables/api";
import { getIdFromIri } from "~~/utils/resource";
import type { Role } from "~~/types/role";

const roleCreateStore = useRoleCreateStore();
const { created, isLoading, violations, error } = storeToRefs(roleCreateStore);

async function create(item: Role) {
  const data = await useCreateItem<Role>("api/roles", item);
  roleCreateStore.setData(data);

  if (!created?.value?.["@id"]) {
    roleCreateStore.setError("Missing item id. Please reload");
    return;
  }

  navigateTo({
    name: "roles-id-edit",
    params: { id: getIdFromIri(created?.value?.["@id"]) },
  });
}

onBeforeUnmount(() => {
  roleCreateStore.$reset();
});
</script>
