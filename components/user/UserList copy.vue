...
const users = computed(() => result.value?.users?.edges?.map((i) => i?.node))

watch(users, (n, o) => {
  console.log(users, (n, o) => {
...
onMounted(async () => {
  ...
  // console.log(columns)
  // NodeService.getCustomersLarge().then((data) => { customers.value = getCustomers(data); console.log(customers.value) });
});
...
<template #subtitle>Listado</template> -->
    <template #content>
      <DataTable v-if="users && users.length" v-model:filters="filters" v-model:selection="selectedCustomers"
        :value="users" paginator :rows="10" dataKey="id" filterDisplay="menu" scrollable scrollHeight="flex"
        :globalFilterFields="['nombre', 'username', 'country.name', 'representative.name', 'balance', 'status']">
        <template #header>
        ...
        <Column v-for="c, i in Object.keys(users[0])" :key="i" :field="c" :header="c" sortable style="min-width: 14rem"
          class="capitalize">
          <template #body="{ data }">
            {{ data[c] }}
          </template>
      ...
      <skeleton-list v-else columns="5" />
    </template>
  </Card>
</template>
