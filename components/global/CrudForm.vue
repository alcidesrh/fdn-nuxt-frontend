<template>
  <div class="w-full h-full @container">
    ...
    <div v-if="formkitSchema.length && item" style="height: 2000px;">
      ...
      <FormKit v-model="item" :actions="false" id="crudForm" type="form" @submit="store.submit">
            ...
            <div class="flex justify-end items-end">
              <slot name="right-header">
                <div class="flex flex-wrap justify-end gap-5 align-middle">
                  ...
                  <CrudBotons @delete="store.remove();" @cancel="$router.push({ name: metadata.routes.list })"
                    :edit="!!item?.id" />
                </div>
              </slot>
            </div>
        ...
        <div class="u-mt-m lg:u-mt-l flex justify-center" style="z-index: 1;">
            ...
            <template v-for="(v, i) in formkitSchema" :key="i">
              <Fieldset v-if="v.wrapper && v.type == 'Fieldset'" v-bind="v.props" class="form-wrapper">
                <template v-if="v.children">
                  <slot v-for="(v2, i2) in v.children" :key="i2" :name="v2.name" :schema="v2" :data="data">
