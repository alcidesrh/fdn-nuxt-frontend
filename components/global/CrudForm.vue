      ...
      <div class="fdn-form-header ">
        <div class="grid @md:flex @md:justify-between w-full items-start">
          <slot name="left-header">
            <span class="u-text-2 font-semibold surface-contrast-600 ">
              {{ entity.item?.id ? 'Editar ' : 'Nuevo ' }} {{entity.name}}
            </span>
          </slot>
            ...
            <slot name="right-header">
              <div class="flex flex-wrap justify-end gap-5 align-middle">
                <CrudButton @submit="submit" @delete="store.remove();"
                  @cancel="$router.push({ name: entity.routes.list })" :edit="!!entity.item?.id" />
              </div>
            </slot>
          </div>
        </div>
