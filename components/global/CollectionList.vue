...
<template>
    <div>
        <div v-if="collection.columns.length">
            ...
            <FormKit type="form" :actions="false" v-model="collection.vars" form-class="block m-auto" :config="{ wrapperClass: 'mb-0!' }">
                <DataTable
                    :rowClass="rowClass"
                    @update:sortField="(i) => store.sortCollection(i)"
                    removableSort
                    tableStyle="min-width:50rem"
                    :sortOrder="collection.orderType == 'ASC' ? 1 : collection.orderType == 'DESC' ? -1 : 0"
                    :value="collection.items"
                    :filterDisplay="collection.hasFilter ? 'row' : undefined"
                    scrollable
                ...
                >
                    ...
                    <div v-for="(c, i) in collection.columns" :key="i">
                        <Column :header="c.label || c.name" :field="c.name" :showFilterMenu="false" :sortable="!!c.sort" alignFrozen="left" :frozen="c.action" :class="c?.class">
                            <template #filter>
