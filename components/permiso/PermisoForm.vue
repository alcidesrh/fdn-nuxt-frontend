<template>
  <form class="py-4" @submit.prevent="emitSubmit">
    ...
    <div class="mb-2">
      ...
      <FormRepeater
        :values="item.hijos"
        @update="(values: any[]) => (item.hijos = values)"
      />
      <div
        v-if="violations?.hijos"
        class="bg-red-100 rounded py-4 px-4 my-2 text-red-700 text-sm"
      >
        {{ violations.hijos }}
      </div>
    ...
    <div class="mb-2">
      <label
        for="permiso_usuarios"
        class="text-gray-700 block text-sm font-bold capitalize"
      >
        usuarios
      </label>
      <FormRepeater
        :values="item.usuarios"
        @update="(values: any[]) => (item.usuarios = values)"
      />
      <div
        v-if="violations?.usuarios"
        class="bg-red-100 rounded py-4 px-4 my-2 text-red-700 text-sm"
      >
        {{ violations.usuarios }}
      </div>
