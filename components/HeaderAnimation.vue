...
<template>
  ...
  <div class="star absolute left-0 h-full w-full" @click="toggle_animation()">
    <div
      v-for="i, index in 500" :id="`star-${index}`" :key="i" to-remove absolute rounded-full bg-slate-3
      class="star-item"
  ...
  <div class="street1 street left-0 z-10 h-50px w-1920px flex items-center justify-between bg-slate-700 shadow-2xl">
    <span v-for="i in 10" :key="i" class="street-middle h-3px w-85px bg-zinc-300" />
  </div>
  ...
  <div class="fixed top-130px">
    <div class="fixed left-50% top-180px top-50% z-40">
      ...
      >
        ...
        <LockIcon
          id="lock" theme="filled" size="50" class="absolute z-2 mt-12" fill="#475569" :stroke-width="5"
          stroke-linecap="square"
        />

        <UnLockIcon
          id="unlock" theme="filled" size="50" class="absolute z-1 mt-12" fill="#475569" :stroke-width="5"
          stroke-linecap="square"
        />
  ...
  <Logo id="logo" class="hidden" />

  <div id="target" class="absolute bottom-0 right-0 z-120 grid max-h-40px max-w-250px w-100% flex items-center border-2 border-slate-3 rounded-full rounded-b-none rounded-r-none border-solid">
    ...
    <div
      v-if="allow_banner_amimation" id="header-bus"
    ...
    <div id="grid-wrap" class="absolute top-0 block h-8px w-100% overflow-hidden bg-slate-700/10">
      <div class="p-grid bg-slate-900/10">
        ...
        <div
          v-for="i, index in 12" id="bus_half_up" :key="index"
  ...
  <div id="target2" class="absolute bottom-0 z-120 grid max-h-40px max-w-250px w-100% flex items-center border-2 border-slate-3 rounded-full rounded-b-none rounded-r-none border-solid">
    <router-link id="logo-wrap" to="/" class="layout-topbar-logo">
      <div class="relative h-80px w-full">
        <WheelIcon id="wheel1" class="absolute bottom-5px left-40px fill-slate-300 text-25px -z-1" />
        <WheelIcon id="wheel2" class="absolute bottom-5px left-165px fill-slate-300 text-25px -z-1" />
