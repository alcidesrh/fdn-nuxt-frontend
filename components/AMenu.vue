<template>
    <div v-if="root" class="menu-wraper" :class="[mode, clases]">
        <Icon name="icon-park-outline:double-down" @click="toggleMenu" class="toggle-menu" :class="{ open: toggle }" mode="svg" size="25" />

        <ol class="layout-menu">
            <AMenu :mode="mode" :menu="menu" />
        </ol>
    </div>
    <li v-else v-for="(menuitem, index) in menu" :key="`_root${index}`">
        <div v-if="menuitem.children" @click="menuRootClick(menuitem, $event)" class="menu-root">
            <div class="menu-icon">
                <Icon :name="menuitem.icon" mode="svg" :class="{ root: root }" v-tooltip="menuitem.label" />
            </div>
            <div class="truncate">{{ menuitem.label }}</div>
            <Icon name="icon-park-outline:down" class="menu-toggle-icon [&>*]:stroke-2" :class="{ open: menuitem.open }" size="24" mode="svg" />
        </div>
        ...
        <NuxtLink v-if="typeof menuitem.to != 'undefined' || menuitem.name" :to="!menuitem.name ? menuitem.to : { name: menuitem.name, params: menuitem.params }">
            <div v-if="menuitem.icon" class="menu-icon">
        ...
        <span v-if="menuitem.subcategory" class="menu-child-category">{{ menuitem.label }}</span>

        <Transition name="layout-submenu">
            <div v-show="menuitem.children && (isActiveRootmenuItem(menuitem) || menuitem.open)">
                <ol>
                    <AMenu :mode="mode" :menu="menuitem.children" :root="false"></AMenu>
                </ol>
            </div>
        </Transition>
...
function isActiveRootmenuItem(menuitem) {
    return menuitem.children.some((item) => item.to === `/${router.currentRoute.value?.label?.replaceAll('-', '/')}` || (item.children && item.children.some((it) => it.to === `/${router.currentRoute.value.label}`)));
}
