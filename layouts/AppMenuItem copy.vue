...
<template>
  ...
  <li v-else class="cursor-pointer" :class="{ 'layout-root-menuitem ': root, 'active-menuitem': item.opened }">
    <div v-if="root && item.visible !== false" class="layout-menuitem-root-text u-p-xs"
      @click="item.opened = !item.opened">
      <!-- <Minus theme="outline" size="18" :stroke-width="2" stroke-linecap="square" /> -->
      <component :is="item.icon" v-if="isComponent(item.icon)" v-bind="{ ...iconProps, strokeWidth: 3 }" />
    ...
    <div class="mx-auto w-80% h-0.1px bg-slate-900/10 hover:bg-slate-900/10" />
