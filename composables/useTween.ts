...
function toggleDay(tl: gsap.core.Timeline, toggle = "day", logo_color = false) {
  if (toggle === "day") {
    ...
    tl.to("#luna", { x: 300, y: 140, duration: 1 }, "<")
      ...
      .to(
        [".cloud1", ".cloud2", ".cloud3", ".cloud4"],
        { opacity: 0, x: 200, y: 140, duration: 1 },
        "<"
      )
...
function paradaAmanecer(tl: gsap.core.Timeline) {
  ...
  tl.to("#target", {
    ...
    .to(
      "#target2",
      { x: nav_width, duration: 4, repeat: -1, repeatDelay: 15 },
      "<"
    ) // parada
...
function busDesarme(tl: gsap.core.Timeline) {
  tl.to(`#star-305, #star-306, #star-307`, {
    top: "50%",
    left: "50%",
  ...
  for (let index = 1; index < 350; index++) {
    if (![305, 306, 307].includes(index))
      tl.to(
        `#star-${index}`,
        { top: "50%", left: "50%", duration: getRamdon(1, 4) },
        "<"
      );
  }
  ...
  tl.to(
    ...
    .to(
      `#star-305`,
      { backgroundColor: "#c2410c", height: "60px", y: -40 },
      "<"
    )
  ...
  tl.to(`#luna`, {
    ...
    [".overlay2", ".overlay", ".telon"],
    { backgroundColor: "white", opacity: 1, duration: 1 },
    "<"
  );
}
function getAxisY() {
  const random = Math.floor(Math.random() * 80);
  if (random < 60) return random;
  return 80 - random;
}
function unlock(tl: gsap.core.Timeline) {
  ...
  for (let index = 0; index < 4; index += 2) {
    tl.to(
      ...
      { width: 20, height: 5, duration: 0.5 },
      "<.2"
    );
    tl.to(
      [nails[index], nails[index + 1]],
      { rotate: 360, y: 300, duration: 1, opacity: 0 },
      "<.5"
    );
  }
  ...
  tl.to(
    ".overlay2",
    { x: window.innerWidth / 2, duration: 4, ease: "slow(0.7,0.7,false)" },
    "<"
  );
  tl.to(
    ".overlay",
...
function starExplotion(tl: gsap.core.Timeline) {
  ...
  tl.to([`#star-205`, `#star-206`, `#star-208`, `#star-209`], {
    width: "0px",
    height: "0px",
    display: "block",
    top: "50%",
  ...
  for (
    let index = 0,
      temp = 45,
      position = "+=0",
      ids = [`#star-205`, `#star-206`, `#star-208`, `#star-209`];
    index < 4;
    index++, temp += 90
  ) {
    if (index === 2) {
    ...
    tl.to(
      ids[index],
      { duration: 0.1, width: "80px", height: "2px", x: -40, y: -1 },
      position
    );
  }
  tl.to(
    [".overlay"],
    { duration: 0.1, borderColor: "#fefce8", borderRightWidth: 0.5 },
    "<"
  );
  tl.to(
    [".overlay2"],
    { duration: 0.1, borderColor: "#fefce8", borderLeftWidth: 0.5 },
    "<"
  );
  ...
  for (let i = 0; i < chars.length; i++) {
    ...
    tl.to(
      chars[i],
      {
        delay: getRamdon(0.05, 0.2),
        zIndex: 100,
        x: window.innerWidth - 50,
        duration: aux,
        rotate: getRamdon(0, 360),
        color: "#fef3c7",
        ease: CustomEase.create(
          "custom",
          "M0,0 C0.14,0 0.242,0.438 0.272,0.561 0.313,0.728 0.354,0.963 0.362,1 0.37,0.985 0.411,0.983 0.472,0.94 0.567,0.871 0.639,0.929 0.652,0.938 0.728,0.988 0.892,0.994 0.911,0.998 0.922,1 1,1 1,1"
        ),
      },
      !i ? "+=0" : `char${i - 1}-=${aux}`
    ).addLabel(`char${i}`);
    tl.to(
      chars[i],
      { top: "50%", y: 50, duration: getRamdon(0.5, 1.5), ease: "bounce.out" },
      `char${i - 1}-=1.4`
    );
    tl.to(
      chars[i],
      {
        x: window.innerWidth - getRamdon(50, 400),
        duration: getRamdon(1.5, 3.5),
      },
      `char${i - 1}-=1`
    );
  }
  ...
  for (let index = 351; index < 500; index++) {
    if (index === 351) {
      tl.to(
        [".overlay2", ".overlay", ".telon"],
        { backgroundColor: "#0f172a", opacity: 0.8, duration: 2.5 },
        "<"
      );
    ...
    if (![305, 306, 307].includes(index))
      tl.to(
        `#star-${index}`,
        {
          ...
          left: "50%",
          x: -(
            (document.querySelector(`#star-${index}`)?.clientWidth || 0) / 2
          ),
          y: -(
            (document.querySelector(`#star-${index}`)?.clientHeight || 0) / 2
          ),
          duration: getRamdon(0, 4),
        },
        "<"
      );
  }
  ...
  tl.to(
    ...
    .to(
      `#star-1`,
      { height: 80, y: -40, width: "1px", x: -0.4, duration: 0.1 },
      "<"
    );
  ...
  tl.to([`#star-0`, `#star-1`], {
    ...
    ease: CustomEase.create(
      "custom",
      "M0,0 C0.031,0.061 0.599,-0.268 0.677,-0.138 0.859,0.165 0.644,1 1,1 "
    ),
  });
  ...
  const space =
    window.innerWidth / (document.querySelectorAll(".char").length * 20);
  const s =
    (window.innerWidth - document.querySelectorAll(".char").length * 22) / 2;
  const axis = [];
  axis.push(s);
  const items = [
    ...document.querySelectorAll(`.name1`),
    ...document.querySelectorAll(`.name2`),
  ];
  for (let i = 1; i < items.length; i++) axis.push(axis[i - 1] + 20 + space);
  let cont = letters.length - 1;
  const aux = cont;
  do {
    const temp = { ...options };
    if (cont === aux) temp.delay = 5;
  ...
  tl.to(
    ["#luna"],
    { backgroundColor: "white", duration: 2, opacity: 1, y: -10 },
    "+=1"
  );
  tl.to(
    Array.from(document.querySelectorAll(".char")),
