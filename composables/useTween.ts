...
function toggleDay(tl: gsap.core.Timeline, toggle = 'day', logo_color = false) {
  if (toggle === 'day') {
    if (logo_color) tl.to(Array.from(document.querySelectorAll('.char')), { duration: 2, color: '#94a3b8' }, '<')
...
function paradaAmanecer(tl: gsap.core.Timeline) {
  tl.to('#header-bus', { display: 'flex', duration: 0 }, '<')
    .to('#logo2', { y: 30, x: -400, scale: 0.8 }, '<') //
    .to('#logo', { x: 400, scale: 0.8 }, '<') //
  ...
  tl.to('#logo', {
    ...
    .to('#logo2', { x: nav_width, duration: 4, repeat: -1, repeatDelay: 15 }, '<') // parada
...
function busDesarme(tl: gsap.core.Timeline) {
  ...
  for (let index = 1; index < 350; index++) {
    if (![305, 306, 307].includes(index)) tl.to(`#star-${index}`, { top: '50%', left: '50%', duration: getRamdon(1, 4) }, '<')
  }
...
function starExplotion(tl: gsap.core.Timeline) {
  ...
  for (let index = 351; index < 500; index++) {
    if (index === 351) {
      tl.to(['.overlay2', '.overlay', '.telon'], { backgroundColor: '#0f172a', opacity: 0.8, duration: 2.5 }, '<')
    ...
    if (![305, 306, 307].includes(index))
      tl.to(
        `#star-${index}`,
        {
          top: '50%',
          left: '50%',
          x: -((document.querySelector(`#star-${index}`)?.clientWidth || 0) / 2),
          y: -((document.querySelector(`#star-${index}`)?.clientHeight || 0) / 2),
  ...
  tl.to([`#star-0`, `#star-1`], {
    ...
    x: -5,
    duration: 0.7,
    ease: CustomEase.create('custom', 'M0,0 C0.031,0.061 0.599,-0.268 0.677,-0.138 0.859,0.165 0.644,1 1,1 '),
  })
    ...
    if (index == 250) {
      ...
      tl.to(
        ['.overlay2', '.overlay'],
        {
          ...
          backgroundColor: '#7dd3fc',
          duration: 3,
          ease: CustomEase.create('custom', 'M0,0,C0,0.408,0.242,0.657,0.295,0.709,0.346,0.76,0.584,1,1,1'),
        },
        '<'
      )
    }
