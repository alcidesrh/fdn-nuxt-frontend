...
export default function (response: OpenAPIV2.Document, entrypointUrl: string): Resource[] {
    ...
    return paths.map((path) => {
        ...
        const requiredFields = get(response, ['definitions', title, 'required'], []) as string[];
        ...
        const fields = fieldNames.map((fieldName) => {
            ...
            return new Field(fieldName, {
                id: null,
                range: null,
                type: getType(get(property, 'type', '') as string, get(property, 'format', '') as string),
                enum: property.enum ? Object.fromEntries(property.enum.map((enumValue: string | number) => [typeof enumValue === 'string' ? inflection.humanize(enumValue) : enumValue, enumValue])) : null,
                reference: null,
                embedded: null,
