...
const isRef = <T extends object>(maybeRef: T | OpenAPIV3.ReferenceObject): maybeRef is T => !('$ref' in maybeRef);
...
const mergeResources = (resourceA: Resource, resourceB: Resource) => {
    ...
    resourceB.readableFields?.forEach((fieldB) => {
        if (!resourceA.readableFields?.some((fieldA) => fieldA.name === fieldB.name)) {
            resourceA.readableFields?.push(fieldB);
        }
    });
    resourceB.writableFields?.forEach((fieldB) => {
        if (!resourceA.writableFields?.some((fieldA) => fieldA.name === fieldB.name)) {
...
const buildResourceFromSchema = (schema: OpenAPIV3.SchemaObject, name: string, title: string, url: string) => {
    ...
    const fields = fieldNames.map((fieldName) => {
        ...
        const field = new Field(fieldName, {
            ...
            range: null,
            type,
            arrayType: type === 'array' && 'items' in property ? getType((property.items as OpenAPIV3.SchemaObject).type || 'string', (property.items as OpenAPIV3.SchemaObject).format) : null,
            enum: property.enum
                ? Object.fromEntries(
                      // Object.values is used because the array is annotated: it contains the __meta symbol used by jsonref.
                      Object.values<string | number>(property.enum).map((enumValue) => [typeof enumValue === 'string' ? inflection.humanize(enumValue) : enumValue, enumValue])
                  )
                : null,
            reference: null,
            embedded: null,
...
const buildOperationFromPathItem = (httpMethod: `${OpenAPIV3.HttpMethods}`, operationType: OperationType, pathItem: OpenAPIV3.OperationObject): Operation => {
...
export default async function (response: OpenAPIV3.Document, entrypointUrl: string): Promise<Resource[]> {
    ...
    paths.forEach((path) => {
        ...
        const showSchema = showOperation ? (get(showOperation, 'responses.200.content.application/json.schema', get(document, `components.schemas[${title}]`)) as OpenAPIV3.SchemaObject) : null;
        const editSchema = editOperation ? (get(editOperation, 'requestBody.content.application/json.schema') as unknown as OpenAPIV3.SchemaObject) : null;
            ...
            resource.parameters = listOperation.parameters
                ...
                .map(
                    (parameter) =>
                        new Parameter(
                            parameter.name,
                            parameter.schema && isRef(parameter.schema) ? (parameter.schema.type ? getType(parameter.schema.type) : null) : null,
