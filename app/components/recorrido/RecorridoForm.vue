<template>
  <form class="py-4" @submit.prevent="emitSubmit">
    ...
    <div class="mb-2">
      ...
      <FormRepeater :values="item.paradas" @update="(values: any[]) => (item.paradas = values)" />
    ...
    <div class="mb-2">
      <label for="recorrido_id" class="text-gray-700 block text-sm font-bold capitalize">
        id
      </label>
      <input id="recorrido_id" v-model="item.id" :class="[
        'mt-1 w-full px-3 py-2 border rounded',
    ...
    <div class="mb-2">
      <label for="recorrido_label" class="text-gray-700 block text-sm font-bold capitalize">
        label
      </label>
      <input id="recorrido_label" v-model="item.label" :class="[
        'mt-1 w-full px-3 py-2 border rounded',
...
if (props.values) {
  item.value = {
    ...
    paradas: props.values.paradas?.map((item: any) => item["@id"] ?? "") ?? [],
  };
}
