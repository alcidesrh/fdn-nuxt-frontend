<template>
  <nuxt-link :to="{ name: 'actions' }" class="text-blue-600 hover:text-blue-800">
    &lt; Back to list
  </nuxt-link>

  <h1 class="text-3xl my-4">Create Action</h1>

  <div v-if="isLoading" class="bg-blue-100 rounded py-4 px-4 text-blue-700 text-sm" role="status">
    Loading...
  </div>

  <div v-if="error" class="bg-red-100 rounded py-4 px-4 my-2 text-red-700 text-sm" role="alert">
    {{ error }}
  </div>

  <Form :errors="violations" @submit="create" />
</template>

<script lang="ts" setup>
import { onBeforeUnmount } from "vue";
import { storeToRefs } from "pinia";
import Form from "~/components/action/ActionForm.vue";
import { useActionCreateStore } from "~/stores/action/create";
import { useCreateItem } from "~/composables/api";
import { getIdFromIri } from "~/utils/resource";
import type { Action } from "~/types/action";

const actionCreateStore = useActionCreateStore();
const { created, isLoading, violations, error } = storeToRefs(actionCreateStore);

async function create(item: Action) {
  const data = await useCreateItem<Action>("actions", item);
  actionCreateStore.setData(data);

  if (!created?.value?.["@id"]) {
    actionCreateStore.setError("Missing item id. Please reload");
    return;
  }

  navigateTo({
    name: "actions-id-edit",
    params: { id: getIdFromIri(created?.value?.["@id"]) },
  });
}

onBeforeUnmount(() => {
  actionCreateStore.$reset();
});
</script>
