<template>
    ...
    <Toast class="z-999 h-fit" position="top-center">
        <template #message="slotProps">
            <div
                class="p-toast-message-text u-py-xs"
                data-p="error"
            ...
            >
                <div class="flex items-start gap-5">
                    ...
                    <div class="grid">
                        <span
                            ...
                            class="p-toast-summary u-mb-xs"
                            data-p="error"
                            data-pc-section="summary"
                            >{{ slotProps.message.summary }}</span
                        >
                        <div
                            class="p-toast-detail font-normal"
                            data-p="error"
                ...
                <div
                    v-if="slotProps.message.file"
                    class="u-my-2xs p-toast-detail flex gap-2 font-semibold"
                    data-p="error"
                    data-pc-section="detail"
                >
                    ...
                    <div>
                        ...
                        <span v-if="slotProps.message.line"
                            >line: {{ slotProps.message.line }}</span
                        >
                    </div>
                </div>
            </div>
...
bus.on((msg: any) => {
    ...
    toast.add({
        ...lifetime,
        detail: msg?.msg || msg || "Mensaje vacio.",
...
error.on((e: any) => {
    ...
    try {
        const { graphQLErrors, networkError } =
            e?.message as ApolloErrorOptions;
    ...
    // cl({ ...lifetime, ...(typeof msg == 'object' ? msg : { detail: msg }), severity: 'error', summary: msg?.summary || 'Error:', file: msg?.file, line: msg?.line });
});
</script>
