...
<template>
  <div>
    <div v-if="collection.columns.length">
      ...
      <FormKit v-model="collection.vars" type="form" :actions="false" form-class="block m-auto" :config="{ wrapperClass: 'mb-0!' }">
        <DataTable
          ...
          :filter-display="collection.hasFilter ? 'row' : undefined"
          scrollable
          scroll-height="700px"
          :class="{ opacity50: collection.loading }"
        ...
        >
          ...
          <template #empty>
            <h5 class="my-5! text-slate-4 m-auto w-fit font-semibold">
              No hay información que mostrar
            </h5>
          </template>

          <div v-for="(c, i) in collection.columns" :key="i">
            <Column :header="c.label || c.name" :field="c.name" :show-filter-menu="false" :sortable="!!c.sort" align-frozen="left" :frozen="c.action" :class="c?.class">
              <template #filter>
          ...
          <Column frozen align-frozen="right" :show-filter-menu="false" :selection-mode="collection.menu == 'selection' ? 'multiple' : undefined" class="action-cell text-center">
            <template #header>
            ...
            <template v-if="collection.menu == 'editar'" #body="{ data }">
              <slot name="action" :data="data">
                <div class="collection-action-wrapper relative flex w-full items-center justify-center gap-4">
                  ...
                  <span class="flex items-center justify-center">
                    <Icon name="icon-park-outline:delete" class="action delete absolute" mode="svg" @click="store.remove(data)" />
                  </span>
                </div>
              </slot>
            </template>
      ...
      <div v-if="collection.pagination?.totalCount" class="u-mb-l u-mt-s flex justify-center">
