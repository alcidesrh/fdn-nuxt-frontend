<template>
  <nuxt-link :to="{ name: 'nacions' }" class="text-blue-600 hover:text-blue-800">
    &lt; Back to list
  </nuxt-link>

  <h1 class="text-3xl my-4">Create Nacion</h1>

  <div v-if="isLoading" class="bg-blue-100 rounded py-4 px-4 text-blue-700 text-sm" role="status">
    Loading...
  </div>

  <div v-if="error" class="bg-red-100 rounded py-4 px-4 my-2 text-red-700 text-sm" role="alert">
    {{ error }}
  </div>

  <Form :errors="violations" @submit="create" />
</template>

<script lang="ts" setup>
import { onBeforeUnmount } from "vue";
import { storeToRefs } from "pinia";
import Form from "~/components/nacion/NacionForm.vue";
import { useNacionCreateStore } from "~/stores/nacion/create";
import { useCreateItem } from "~/composables/api";
import { getIdFromIri } from "~/utils/resource";
import type { Nacion } from "~/types/nacion";

const nacionCreateStore = useNacionCreateStore();
const { created, isLoading, violations, error } = storeToRefs(nacionCreateStore);

async function create(item: Nacion) {
  const data = await useCreateItem<Nacion>("nacions", item);
  nacionCreateStore.setData(data);

  if (!created?.value?.["@id"]) {
    nacionCreateStore.setError("Missing item id. Please reload");
    return;
  }

  navigateTo({
    name: "nacions-id-edit",
    params: { id: getIdFromIri(created?.value?.["@id"]) },
  });
}

onBeforeUnmount(() => {
  nacionCreateStore.$reset();
});
</script>
