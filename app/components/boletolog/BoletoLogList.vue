<template>
  <div class="flex items-center justify-between">
    ...
    <nuxt-link :to="{ name: 'boletologs-create' }"
      ...
      <tbody>
        <tr v-for="item in items" :key="item['@id']" class="border-b">
          <td class="px-6 py-4 text-sm">
            <nuxt-link :to="{ name: 'boletologs-id', params: { id: getIdFromIri(item['@id']) } }"
          ...
          <td class="px-6 py-4 text-sm">
            <nuxt-link :to="{ name: 'boletologs-id-edit', params: { id: getIdFromIri(item['@id']) } }"
              class="px-6 py-2 bg-green-600 text-white text-xs rounded shadow-md hover:bg-green-700">
              Edit
            </nuxt-link>
          </td>
  ...
  <div v-if="view" class="flex justify-center">
    <nav aria-label="Page navigation">
      <ul class="flex list-style-none">
        ...
        <li :class="{ disabled: !pagination.next }">
          <nuxt-link :to="{ name: 'boletologs-page-page', params: { page: pagination.last } }" :class="!pagination.next
              ? 'text-gray-500 pointer-events-none'
              : 'text-gray-800 hover:bg-gray-200'
