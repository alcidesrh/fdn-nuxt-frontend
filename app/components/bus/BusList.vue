      ...
      <tbody>
        <tr v-for="item in items" :key="item['@id']" class="border-b">
          <td class="px-6 py-4 text-sm">
            <nuxt-link :to="{ name: 'buss-id', params: { id: getIdFromIri(item['@id']) } }"
          ...
          <td class="px-6 py-4 text-sm">
            <template v-if="router.hasRoute('asientos-id')">
              <nuxt-link v-for="asiento in item.asientos" :key="asiento"
                :to="{ name: 'asientos-id', params: { id: asiento } }" class="text-blue-600 hover:text-blue-800">
                {{ asiento }}
            ...
            <template v-else>
              <p v-for="asiento in item.asientos" :key="asiento">
                {{ asiento }}
              </p>
            </template>
          </td>
          ...
          <td class="px-6 py-4 text-sm">
            <nuxt-link v-if="router.hasRoute('pilotos-id')" :to="{ name: 'pilotos-id', params: { id: item.piloto } }"
              class="text-blue-600 hover:text-blue-800">
              {{ item.piloto }}
            </nuxt-link>
          ...
          <td class="px-6 py-4 text-sm">
            <nuxt-link :to="{ name: 'buss-id-edit', params: { id: getIdFromIri(item['@id']) } }"
              class="px-6 py-2 bg-green-600 text-white text-xs rounded shadow-md hover:bg-green-700">
              Edit
            </nuxt-link>
          </td>
  ...
  <div v-if="view" class="flex justify-center">
    <nav aria-label="Page navigation">
      <ul class="flex list-style-none">
        ...
        <li :class="{ disabled: !pagination.next }">
          <nuxt-link :to="{ name: 'buss-page-page', params: { page: pagination.last } }" :class="!pagination.next
              ? 'text-gray-500 pointer-events-none'
              : 'text-gray-800 hover:bg-gray-200'
