...
import { Resource } from '../graphql/Resource.js';
...
import type { ExpandedClass, ExpandedDoc, Entrypoint, ExpandedOperation, ExpandedRdfProperty, RequestInitExtended } from './types.js';
...
function getTitleOrLabel(obj: ExpandedOperation): string {
    const a = obj['http://www.w3.org/2000/01/rdf-schema#label'] ?? obj['http://www.w3.org/ns/hydra/core#title'] ?? null;
 ...
 */
function findSupportedClass(docs: ExpandedDoc[], classToFind: string): ExpandedClass {
    const supportedClasses = get(docs, '[0]["http://www.w3.org/ns/hydra/core#supportedClass"]') as ExpandedClass[] | undefined;
...
function findRelatedClass(docs: ExpandedDoc[], property: ExpandedRdfProperty): ExpandedClass {
    ...
    if (Array.isArray(property['http://www.w3.org/2000/01/rdf-schema#range'])) {
        for (const range of property['http://www.w3.org/2000/01/rdf-schema#range']) {
            const onProperty = get(range, '["http://www.w3.org/2002/07/owl#equivalentClass"][0]["http://www.w3.org/2002/07/owl#onProperty"][0]["@id"]') as unknown as string;
            const allValuesFrom = get(range, '["http://www.w3.org/2002/07/owl#equivalentClass"][0]["http://www.w3.org/2002/07/owl#allValuesFrom"][0]["@id"]') as unknown as string;
    ...
    for (const entrypointSupportedOperation of property['http://www.w3.org/ns/hydra/core#supportedOperation'] || []) {
        ...
        const returns = get(entrypointSupportedOperation, '["http://www.w3.org/ns/hydra/core#returns"][0]["@id"]') as string | undefined;
    ...
    return fetchEntrypointAndDocs(entrypointUrl, options).then(
        ({ entrypoint, docs, response }) => {
            const resources = [],
                fields = [],
                operations = [];
            const title = get(docs, '[0]["http://www.w3.org/ns/hydra/core#title"][0]["@value"]', 'API Platform') as string;

            const entrypointType = get(entrypoint, '[0]["@type"][0]') as string | undefined;
            if (!entrypointType) {
            ...
            for (const properties of entrypointClass['http://www.w3.org/ns/hydra/core#supportedProperty']) {
                ...
                const property = get(properties, '["http://www.w3.org/ns/hydra/core#property"][0]') as ExpandedRdfProperty | undefined;

                if (!property) {
                    continue;
                }

                const url = get(entrypoint, `[0]["${property['@id']}"][0]["@id"]`) as string | undefined;
                ...
                for (const supportedProperties of relatedClass['http://www.w3.org/ns/hydra/core#supportedProperty']) {
                    const supportedProperty = get(supportedProperties, '["http://www.w3.org/ns/hydra/core#property"][0]') as unknown as ExpandedRdfProperty;
                    const id = supportedProperty['@id'];
                    const range = get(supportedProperty, '["http://www.w3.org/2000/01/rdf-schema#range"][0]["@id"]', null) as unknown as string;

                    const field = new Field(supportedProperties['http://www.w3.org/ns/hydra/core#title'][0]['@value'] ?? supportedProperty['http://www.w3.org/2000/01/rdf-schema#label'][0]['@value'], {
                        id,
                        range,
                        type: getType(id, range),
                        reference:
                            'http://www.w3.org/ns/hydra/core#Link' === get(supportedProperty, '["@type"][0]')
                        ...
                        embedded:
                            'http://www.w3.org/ns/hydra/core#Link' !== get(supportedProperty, '["@type"][0]')
                                ...
                                : null,
                        required: get(supportedProperties, '["http://www.w3.org/ns/hydra/core#required"][0]["@value"]', false) as boolean,
                        description: get(supportedProperties, '["http://www.w3.org/ns/hydra/core#description"][0]["@value"]', '') as string,
                        maxCardinality: get(supportedProperty, '["http://www.w3.org/2002/07/owl#maxCardinality"][0]["@value"]', null) as number | null,
                        deprecated: get(supportedProperties, '["http://www.w3.org/2002/07/owl#deprecated"][0]["@value"]', false) as boolean
                    });
                ...
                if (property['http://www.w3.org/ns/hydra/core#supportedOperation']) {
                    for (const entrypointOperation of property['http://www.w3.org/ns/hydra/core#supportedOperation']) {
                        ...
                        const operation = new Operation(getTitleOrLabel(entrypointOperation), type, {
                            method,
                            ...
                            deprecated: get(entrypointOperation, '["http://www.w3.org/2002/07/owl#deprecated"][0]["@value"]', false) as boolean
                        });
                ...
                for (const supportedOperation of relatedClass['http://www.w3.org/ns/hydra/core#supportedOperation'] || []) {
                    ...
                    const operation = new Operation(getTitleOrLabel(supportedOperation), type, {
                        method,
                        ...
                        deprecated: get(supportedOperation, '["http://www.w3.org/2002/07/owl#deprecated"][0]["@value"]', false) as boolean
                    });
                ...
                const resource = new Resource(guessNameFromUrl(url, entrypointUrl), url, {
                    id: relatedClass['@id'],
                    title: get(relatedClass, '["http://www.w3.org/ns/hydra/core#title"][0]["@value"]', '') as string,
                    fields: resourceFields,
                    readableFields,
                    writableFields,
                    operations: resourceOperations,
                    deprecated: get(relatedClass, '["http://www.w3.org/2002/07/owl#deprecated"][0]["@value"]', false) as boolean
                });
