...
/**
 * Sends a JSON-LD request to the fdn.
 */
export default async function fetchJsonLd(url: string, options: RequestInitExtended = {}): Promise<ResponseDocument | EmptyResponseDocument> {
...
function setHeaders(options: RequestInitExtended): RequestInit {
    ...
    let headers: HeadersInit = typeof options.headers === 'function' ? options.headers() : options.headers;
